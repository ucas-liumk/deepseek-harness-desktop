...
describe('desktop release workflow', () => {
  ...
  it('runs fixed desktop regressions before native builds and tests Rust after the real sidecar exists', () => {
    ...
    const linuxPrerequisitesSource = stringField(linuxPrerequisites, 'run')
    expect(linuxPrerequisitesSource).toContain('openbox')
    expect(linuxPrerequisitesSource).toContain('x11-utils')
    expect(linuxPrerequisitesSource).toContain('xdotool')
    expect(linuxPrerequisitesSource).toContain('xvfb')
  ...
  it('installs, starts, closes, checks, and purges the final Linux deb', () => {
    const prerequisites = step(job(jobs, 'build'), 'Install Linux desktop prerequisites')
    const linuxStep = step(job(jobs, 'build'), 'Package and verify unsigned Linux x64 deb')
    const source = stringField(linuxStep, 'run')
    expect(stringField(prerequisites, 'run')).toMatch(/^\s*wmctrl \\/m)
    ...
    expect(source).toContain('wmctrl -i -c "$window"')
    expect(source).toContain('WM_PROTOCOLS')
    expect(source).toContain('WM_DELETE_WINDOW')
    expect(source).toContain('xdotool search --all --onlyvisible --maxdepth 2')
    expect(source).not.toContain('xdotool windowclose "$window"')
    expect(source).not.toContain('xdotool key --clearmodifiers alt+F4')
    expect(source).toContain('process_state()')
    expect(source).toContain("''|Z*|X*) return 1")
    expect(source).toContain('process_is_live "$app_pid"')
    expect(source).toContain('xprop -id "$window"')
    expect(source).toContain('ps -o pid,ppid,pgid,sid,stat,etime,wchan:32,args -p "$app_pid"')
    expect(source).toContain('openbox >"$openbox_log" 2>&1 &')
    expect(source).toContain('openbox_pid=$!')
    expect(source).toContain('xprop -root _NET_SUPPORTING_WM_CHECK')
    expect(source).toContain('grep -Eq \'window id # 0x[0-9a-fA-F]+\'')
    expect(source.match(/kill -0 "\$openbox_pid"/g)).toHaveLength(2)
    expect(source).not.toContain("xdotool search --name '^Openbox$'")
    expect(source).toContain('terminate_and_reap "$openbox_pid"')
    expect(source).toContain('wait "$pid" 2>/dev/null || true')
    expect(source.indexOf('trap cleanup_smoke EXIT')).toBeLessThan(source.indexOf('openbox >"$openbox_log" 2>&1 &'))
    expect(source.indexOf('openbox >"$openbox_log" 2>&1 &')).toBeLessThan(source.indexOf('"$main" >"$log" 2>&1 &'))
    expect(source.indexOf('openbox_ready=1')).toBeLessThan(source.indexOf('"$main" >"$log" 2>&1 &'))
    expect(source.indexOf('terminate_and_reap "$app_pid"')).toBeLessThan(
      source.indexOf('terminate_and_reap "$openbox_pid"'),
    )
