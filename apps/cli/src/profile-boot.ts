...
import type { DesktopApiAuthorizer } from './desktop-api-auth.ts'
...
export interface RunProfileOptions {
  ...
  /** Installed-runtime module anchor for a closed executable's bundled plugins. */
  bareModuleBaseUrl?: string
  /** Boolean-only desktop authentication oracle; its token stays closure-private. */
...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  ...
  const ctx = await boot(NAME, rootConfig, structuredClone(allPatches(composed)), (hostCtx) => {
    ...
    if (options.desktopApiAuthorizer !== undefined) {
      hostCtx.provide('desktopApiAuthorizer', options.desktopApiAuthorizer)
    }
    ...
    try {
      ...
      if (ctx.get('hmr') === undefined) {
        ...
        await ctx.loader.create({
          ...
          config: {
            base: pathToFileURL(composed.profile.dir).href,
            root: [],
          },
        })
      }
