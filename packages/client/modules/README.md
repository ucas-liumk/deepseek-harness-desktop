...
The Node half scans enabled Loader entries for web `dsh.client` packages, resolves each package from the config tree and then the Loader's installed-runtime base, resolves its `exports["./client"]`, hashes the built bundle into the boot graph, and serves it with its source map under `/plugins`. The second anchor keeps closed executables inside their virtual filesystem instead of relying on operating-system links to virtual paths. Source launch maps host imports to TypeScript source but still consumes this built client export; missing files share one build instruction followed by a package/path list, while unrelated filesystem errors remain separate failures.
