...
Node 侧会扫描已启用的 Loader 配置项以发现 web `dsh.client` 包，先从配置树、再从 Loader 的已安装运行时基址解析每个包，解析其 `exports["./client"]`，把构建后的 bundle 哈希写入启动图，并通过 `/plugins` 提供该文件及其 sourcemap。第二个锚点让封闭式可执行文件始终在自身虚拟文件系统内解析，不依赖指向虚拟路径的操作系统链接。源码启动会把宿主侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出；缺失文件共享一条构建说明，随后以包／路径列表列出各项，而无关的文件系统错误仍是独立故障。
