...
describe('current selection (migrated from ui-layout, arbitrated into the list snapshot)', () => {
  ...
  it.each([
    ['invalid shape', encodeURIComponent(JSON.stringify({ sessionId: 7 }))],
    ['empty id', encodeURIComponent(JSON.stringify({ sessionId: '' }))],
    ['unexpected fields', encodeURIComponent(JSON.stringify({ sessionId: 's1', prompt: 'secret' }))],
    ['mismatched child address', encodeURIComponent(JSON.stringify({
      sessionId: 's1',
      subagentAddress: {
        parentSessionId: 'parent', childSessionId: 's2', mode: 'continuable',
      },
    }))],
    ['oversized value', encodeURIComponent(JSON.stringify({ sessionId: 'x'.repeat(2_049) }))],
    ['malformed encoding', '%E0%A4%A'],
  ])('rejects a macOS desktop selection cookie with %s', (_case, encoded) => {
    vi.stubGlobal('__DSH_DESKTOP_PLATFORM__', 'macos')
    const cookies = stubCookieDocument(`dsh.desktop.sessions.current=${encoded}`)
    const storage = new Map<string, string>()
    vi.stubGlobal('localStorage', {
