...
macOS 桌面端每次都会在不同端口启动 loopback Host。因此，`SessionRuntime` 会先从经校验且编码后不超过 2 KiB 的 selection cookie，为该端口独立的 `localStorage` 播种，并把后续选择同步回应用 WebView 的 loopback 主机级 cookie。cookie 只接受当前 Session／subagent selection 结构。普通浏览器继续保持既有的同源本地行为。桌面端首次迁移尚无持久 selection 时，`WorkspaceRuntime` 会打开最近活跃 Workspace 中最近更新、未归档的非空白 Session，而不会创建空白启动行。
