...
import { isTrustedApiRequest, type DesktopApiAuthorizer } from './api-request-trust.ts'
...
export class HostConnectionService extends Service implements HostConnectionHandle {
  ...
  constructor(
    ...
    private readonly desktopApiAuthorizer?: DesktopApiAuthorizer,
  ) {
    ...
    return {
      fetch: (request) => {
        ...
        if (interceptor.options.authority === 'loopback'
          && !isTrustedApiRequest(request, [], 'http', this.desktopApiAuthorizer)) {
  ...
  private register(
    ...
    const route: WebRoute = {
      ...
      handler: async (req, res) => {
        if (!isTrustedApiRequest(req, trustedHosts, 'http', this.desktopApiAuthorizer)) {
          res.writeHead(403)
          res.end('forbidden')
          return
        }
