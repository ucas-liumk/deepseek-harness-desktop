...
该命令只由 Web bundle 挂载。只有 `/export` 返回成功时，本地 `command/executed` 确认才会在提交命令的浏览器中触发斜杠下载；其他标签页仍会渲染持久命令行，但不会重复执行浏览器副作用。Header 按钮直接调用同一个控制器。两种入口都会先发出 `HEAD` 预检，再把 GET URL 交给浏览器下载管理器，JavaScript 不会缓冲 ZIP；它们共用并发折叠、插件释放时取消预检、准备阶段错误处理、浏览器保存行为和同一个 Modal。在已认证的桌面模式下，WebView 会为预检加入本次启动 bearer。成功响应必须包含小写的 256 位票据；控制器只把该票据加入 GET URL。Host 仅允许在 30 秒内使用票据一次，并要求导出路径和其余 query 完全匹配。在 macOS 桌面端，控制器随后最多等待两分钟，直到一个最终原生 WebView 事件同时给出完成状态和系统“下载”文件夹实际采用的防重名文件名。普通浏览器保留原有的不含票据 URL，并在交给下载管理器后立即完成控制器操作。

Host 下载端点会在 `readRaw` 前 flush 活动的根 Session，因此斜杠命令触发的 ZIP 会包含启动下载的 `command/run` 与 `command/done` 事件对。冷持久化 Session 不需要 flush。

弹窗报告准备中、浏览器开始下载、macOS 桌面下载完成或失败。关闭弹窗不会取消正在进行的下载；该操作随后完成时也不会重新打开弹窗。每个 Session 同时只允许一项下载，重复操作会共用该任务。
...
- 这是 WebView／浏览器下载，不是 Host 路径写入。普通浏览器自行选择目标位置。macOS 桌面 WebView 通过系统下载管理器保存，只返回实际文件名，不返回绝对路径，也不提供原生文件夹能力。
- 预检会报告 ZIP 开始流式传输前发现的失败。桌面模式下，票据 header 缺失或格式不正确时，浏览器移交前即失败；票据过期、重放或绑定不同时，服务返回 HTTP 403。在 macOS 桌面端，一个最终 WebView 事件同时提供成功位和实际文件名；完成数据缺失、格式不正确或不完整时，两分钟后判定失败。普通浏览器在接受 GET 后遇到的子 Session 或附件读取失败，仍由浏览器下载管理器报告，不通过弹窗报告。
